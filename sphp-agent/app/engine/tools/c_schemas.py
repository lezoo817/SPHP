...
def register_c_tools() -> None:
    ...
    ToolRegistry.register(ToolSchema(
        ...
        parameters={
            "properties": {
                "appointment_id": {"type": "integer", "description": "挂号订单ID（选填，不传返回列表）"},
    ...
    ToolRegistry.register(ToolSchema(
        ...
        description="查询问诊记录列表或详情（不带 consultation_id 返回列表，带 id 返回详情及消息记录）",
        parameters={
            "properties": {
    ...
    ToolRegistry.register(ToolSchema(
        ...
        description="查询处方列表或详情（不带 prescription_id 返回列表，带 id 返回详情及药品明细）",
        parameters={
            "properties": {
    ...
    ToolRegistry.register(ToolSchema(
        ...
        description="以通俗语言解读处方药品。不可修改处方内容；Agent 无权开方；标注AI建议仅供参考。",
        parameters={
            "properties": {
    ...
    ToolRegistry.register(ToolSchema(
        ...
        parameters={
            "properties": {
                "allergy_id": {"type": "integer", "description": "过敏记录ID（选填，不传=新增）"},
    ...
    ToolRegistry.register(ToolSchema(
        ...
        parameters={
            "properties": {
                "history_id": {"type": "integer", "description": "既往史记录ID（选填，不传=新增）"},
    ...
    ToolRegistry.register(ToolSchema(
        ...
        parameters={
            "properties": {
                ...
                "action": {"type": "string", "enum": ["PAUSE", "RESUME", "COMPLETE"], "description": "操作类型"},
            },
    ...
    ToolRegistry.register(ToolSchema(
        ...
        parameters={
            "properties": {
                "action": {"type": "string", "enum": ["list", "read"], "description": "操作类型"},
                "notification_id": {"type": "integer", "description": "通知ID（action=read 时必填）"},
            },
            "required": ["action"],
        },
        scope=ToolScope.C_END,
