...
INTENT_LABELS = {
    ...
    "chitchat": "闲聊寒暄：打招呼、感谢、告别等社交性表达（不回答知识性问题）",
    "out_of_scope": (
        "非医疗知识提问：与医疗健康无关的知识/任务类请求"
        "（如写作、翻译、编程、时事、天气、娱乐等），应说明职责边界不予回复"
    ),
    "health": (
...
4. 症状描述优先归类为"triage"
5. 打招呼/感谢/告别等社交性表达归类为"chitchat"
6. 与医疗健康无关的知识/任务类提问（写作/翻译/编程/时事/天气等）归类为"out_of_scope"
...
5. 用户明确提出与医疗健康无关的知识/任务类请求（写作/翻译/编程/时事/天气等）时，切换到"out_of_scope"
6. 无法判断时保持当前意图 {last_intent}
