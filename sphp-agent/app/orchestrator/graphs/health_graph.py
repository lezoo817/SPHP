...
绑定健康档案场景工具白名单（10 个）：L1 查询类 5 个（档案/报告/用药
计划/随访/通知）+ L2 变更类 5 个（过敏史/既往史/报告录入/用药计划
更新/随访确认）。M8-2 补齐场景五工具路由——此前 10 个健康工具全部
注册但无 intent 路由、4 业务子图白名单均不含（仅 triage 含
query_health_record 1 个），LLM 无路径可达。
