import type { NotificationItem, NotificationReadResult, PageData } from '../typings/api';
import { request } from './request';

/** 通知列表的可选筛选条件。 */
export interface NotificationListQuery { patientId?: number; read?: boolean; pageNo?: number; pageSize?: number; }

/**
 * 构建通知列表路径，避免将空筛选条件传给后端。
 * @param query 可选就诊人、已读状态与分页条件
 * @returns C 端通知列表请求路径
 */
