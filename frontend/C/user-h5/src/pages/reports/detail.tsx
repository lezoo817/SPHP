...
import { isReportInterpretationPending } from '../../utils/report';

/** 展示医生病历报告正文、静态信息占位和已生成的报告解读。 */
export default function ReportDetailPage() {
  ...
  async function loadReportDetail() {
    ...
    setNotice('');
    setLoading(true);
    const [detailResult, interpretationResult] = await Promise.allSettled([getReport(id), getReportInterpretation(id)]);
  ...
  return <main className="subpage report-detail-page"><PageHeader title="报告详情" backPath={`/reports${location.search}`} /><section className="subpage-content">
    ...
    {!loading && detail && <><header className="report-detail-title"><FileText size={28} /><div><h2>{detail.departmentName || '医生报告'}</h2><p>{detail.doctorName || '医生待确认'} · 完成于 {formatReportDate(detail.completedAt)}</p></div></header><section className="report-document"><div className="report-document__header"><Stethoscope size={23} /><div><b>医生病历报告</b><span>报告编号：暂未提供</span></div></div><dl className="report-document__meta"><div><dt>所属科室</dt><dd>{detail.departmentName || '暂未提供'}</dd></div><div><dt>接诊医生</dt><dd>{detail.doctorName || '暂未提供'}</dd></div><div><dt>开始时间</dt><dd>{formatReportDate(detail.startedAt)}</dd></div><div><dt>完成时间</dt><dd>{formatReportDate(detail.completedAt)}</dd></div><div><dt>检查项目</dt><dd>暂未提供</dd></div><div><dt>影像指标</dt><dd>暂未提供</dd></div></dl><section className="report-document__content"><h3>医生病历正文</h3><p>{detail.doctorNote}</p></section><p className="report-document__updated">最后更新：{formatReportDate(detail.updatedAt)}</p></section>{interpretation && <section className="report-interpretation"><div><Sparkles size={22} /><h2>报告解读</h2></div><p>{interpretation.content}</p><small>{interpretation.disclaimer}</small><time>生成时间：{formatReportDate(interpretation.generatedAt)}</time></section>}{isInterpretationPending && <section className="report-interpretation pending"><AlertCircle size={22} /><div><h2>报告解读正在生成</h2><p>医生解读准备完成后会在此处显示。</p></div></section>}</>}
