...
export default function HealthRecordPage() {
  ...
  return <main className="subpage"><PageHeader title="健康档案" /><section className="subpage-content">{loading ? <p className="empty-state">正在读取健康档案...</p> : record && <><button className="health-summary health-summary--switch" type="button" onClick={() => setPatientOpen(true)}><ClipboardList size={25} /><div><h2>{record.profile.name}的健康档案</h2><p>{record.summary || '暂未记录健康信息'}</p></div><ChevronRight size={20} aria-hidden="true" /></button><RecordSection title="过敏史" emptyText="暂无过敏史" onAdd={() => openEditor({ type: 'allergy' })}>{record.allergies.map((item) => <RecordItem key={item.id} title={item.allergen} detail={item.reaction || '未填写过敏反应'} onEdit={() => openEditor({ type: 'allergy', item })} />)}</RecordSection><RecordSection title="既往史" emptyText="暂无既往史" onAdd={() => openEditor({ type: 'history' })}>{record.medicalHistories.map((item) => <RecordItem key={item.id} title={item.content} detail={item.occurredAt || '未填写发生日期'} onEdit={() => openEditor({ type: 'history', item })} />)}</RecordSection></>}</section>{patientOpen && <Dialog title="切换就诊人" onClose={() => setPatientOpen(false)}>{members.map((member) => <button className="choice-row" key={member.patientId} type="button" onClick={() => selectPatient(member.patientId)}><span>{member.name}</span><small>{member.relationName || member.relation}{member.patientId === patientId ? ' · 当前选择' : ''}</small></button>)}</Dialog>}{notice && <div className="toast" onClick={() => setNotice('')}>{notice}</div>}{editor?.type === 'allergy' && <Dialog title={editor.item ? '编辑过敏史' : '添加过敏史'} onClose={() => setEditor(null)}><form className="form-stack" onSubmit={saveAllergy}><label>过敏原<input value={allergen} maxLength={128} onChange={(event) => setAllergen(event.target.value)} /></label><label>过敏反应<input value={reaction} maxLength={512} placeholder="可选，如皮疹" onChange={(event) => setReaction(event.target.value)} /></label><button className="primary-button" disabled={submitting} type="submit">{submitting ? '保存中...' : '保存过敏史'}</button></form></Dialog>}{editor?.type === 'history' && <Dialog title={editor.item ? '编辑既往史' : '添加既往史'} onClose={() => setEditor(null)}><form className="form-stack" onSubmit={saveHistory}><label>既往史<textarea value={content} maxLength={2000} rows={4} onChange={(event) => setContent(event.target.value)} /></label><label>发生日期<input type="date" value={occurredAt} onChange={(event) => setOccurredAt(event.target.value)} /></label><button className="primary-button" disabled={submitting} type="submit">{submitting ? '保存中...' : '保存既往史'}</button></form></Dialog>}</main>;
}
