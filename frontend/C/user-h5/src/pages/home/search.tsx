...
export default function MedicalSearchPage() {
  ...
  async function search() {
    ...
    try {
      ...
      const doctorPages = await Promise.all(allDepartments.map((department) => getDoctors(hospitalId, department.id, new Date().toISOString().slice(0, 10)).then((page) => page.records.map((doctor) => ({ ...doctor, departmentName: department.name })))));
  ...
  return <main className="subpage discovery-page">
    ...
    <section className="subpage-content medical-search-page">
      ...
      {searched && <>
        ...
        {tab === 'doctor' && <section>{doctors.map((doctor) => <button className="doctor-result" key={doctor.id} type="button" onClick={() => navigate(`/assistant/book?doctorId=${doctor.id}`)}><span className="doctor-avatar">{doctor.name.slice(0, 1)}</span><div><b>{doctor.name} <small>{doctor.title || '医生'}</small></b><p>{doctor.departmentName} · {doctor.specialty || '暂无专长说明'}</p><em>{doctor.availableCount > 0 ? `可约 ${doctor.availableCount}` : '暂时无号'}</em></div></button>)}{!doctors.length && <p className="empty-state">未找到相关医生</p>}</section>}
