...
export function buildHealthTodos(sources: PatientHealthSource[], nowMillis = Date.now()): HealthTodo[] {
  const todos = sources.flatMap((source) => [
    ...
    // 仅接诊中的无挂号在线问诊需要患者继续处理；已完成记录属于历史信息，不进入待办区。
    ...(source.consultations || []).filter((item) => item.status === 'IN_PROGRESS'
      && (item.appointmentId === undefined || item.appointmentId === null)).map((item) => ({ id: item.id, type: 'CONSULTATION' as const, patientId: source.patientId, patientName: source.patientName, title: `${item.doctorName || '医生待确认'} · 在线问诊`, detail: '医生接诊中', occurredAt: item.updatedAt })),
