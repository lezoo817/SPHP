...
class OrderServiceImplTest {
    ...
    /** 验证订单列表使用本人患者和默认分页。 */ @Test void listOrdersUsesDefaultPatientAndPage(){
        ...
        when(mapper.selectOrderList(20001L,"PENDING_PAYMENT",null,20,0)).thenReturn(List.of(new OrderListRecord(15001L,"健康药房","PENDING_PAYMENT","PENDING_SHIPMENT",null,7000,OffsetDateTime.now()))); when(mapper.countOrderList(20001L,"PENDING_PAYMENT",null)).thenReturn(1L);
        DrugOrderPageVO result=service.listDrugOrders(null,"PENDING_PAYMENT",null,null,null);
        assertEquals(1L,result.getTotal()); assertEquals(15001L,result.getRecords().getFirst().getId()); assertEquals(20,result.getPageSize());
    }
