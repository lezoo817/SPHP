...
class OrderControllerTest {
    ...
    /** 验证购药订单列表将订单名关键词传递给服务层。 */ @Test void listOrders() throws Exception { OrderService s=mock(OrderService.class); when(s.listDrugOrders(null,null,null,"阿莫西林",null,null)).thenReturn(DrugOrderPageVO.builder().pageNo(1).pageSize(20).total(0).records(List.of()).build()); mvc(s,mock(PaymentService.class),mock(CIdempotencyService.class)).perform(get("/c/v1/drug-orders").param("keyword","阿莫西林")).andExpect(status().isOk()).andExpect(jsonPath("$.data.total").value(0)); }
