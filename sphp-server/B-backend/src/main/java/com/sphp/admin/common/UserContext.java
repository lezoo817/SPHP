...
public record UserContext(BUser user, Long deptId) {
    ...
    /** @return 当前用户角色（ADMIN / DEPT_HEAD / DOCTOR） */
    public String role() {
        return user.getRole();
    }

    /** @return 当前用户所属医院 ID（始终非空） */
    public Long hospitalId() {
        return user.getHospitalId();
    }

    /** @return 当前用户关联的 doctor.id（ADMIN 可能为 null） */
     ...
     * 转换为数据权限范围（三角色通用）。
     *
     * @return 由当前上下文派生的 DataScope；doctorId 为 null（如 ADMIN）时 deptId 同步为 null
     */
